...
MEMORY
    ...
    RAM (wx) : ORIGIN = 0x24000000, LENGTH = (320 * 1K)
    BACKUP_SRAM : ORIGIN = (947912704), LENGTH = (4096) EXTMEM : ORIGIN = (2415919104), LENGTH = (268435456) SRAM1 : ORIGIN = (805306368), LENGTH = (16384) SRAM2 : ORIGIN = (805322752), LENGTH = (16384) SRAM4 : ORIGIN = (939524096), LENGTH = (16384) DTCM : ORIGIN = (536870912), LENGTH = (131072) ITCM : ORIGIN = (0), LENGTH = (65536)
 ...
 *(".kernel_noinit.*")
    ...
    __kernel_ram_end = 0x24000000 + (320 * 1K);
...
eth_stm32 (NOLOAD) : ALIGN_WITH_INPUT
{
    . = ABSOLUTE((805322752));
    *(.eth_stm32_desc)
    . = ABSOLUTE((805322752)) + 256;
    *(.eth_stm32_buf)
    . = ABSOLUTE((805322752)) + 16K;
} > "SRAM2" AT > "SRAM2"
.intList :
{
 KEEP(*(.irq_info*))
 ...
 KEEP(*(.gnu.attributes))
    ...
    BACKUP_SRAM (NOLOAD) : { __BACKUP_SRAM_start = .; KEEP(*(BACKUP_SRAM)) KEEP(*(BACKUP_SRAM.*)) __BACKUP_SRAM_end = .; } > BACKUP_SRAM __BACKUP_SRAM_size = __BACKUP_SRAM_end - __BACKUP_SRAM_start; __BACKUP_SRAM_load_start = LOADADDR(BACKUP_SRAM); EXTMEM (NOLOAD) : { __EXTMEM_start = .; KEEP(*(EXTMEM)) KEEP(*(EXTMEM.*)) __EXTMEM_end = .; } > EXTMEM __EXTMEM_size = __EXTMEM_end - __EXTMEM_start; __EXTMEM_load_start = LOADADDR(EXTMEM); SRAM1 (NOLOAD) : { __SRAM1_start = .; KEEP(*(SRAM1)) KEEP(*(SRAM1.*)) __SRAM1_end = .; } > SRAM1 __SRAM1_size = __SRAM1_end - __SRAM1_start; __SRAM1_load_start = LOADADDR(SRAM1); SRAM2 (NOLOAD) : { __SRAM2_start = .; KEEP(*(SRAM2)) KEEP(*(SRAM2.*)) __SRAM2_end = .; } > SRAM2 __SRAM2_size = __SRAM2_end - __SRAM2_start; __SRAM2_load_start = LOADADDR(SRAM2); SRAM4 (NOLOAD) : { __SRAM4_start = .; KEEP(*(SRAM4)) KEEP(*(SRAM4.*)) __SRAM4_end = .; } > SRAM4 __SRAM4_size = __SRAM4_end - __SRAM4_start; __SRAM4_load_start = LOADADDR(SRAM4); DTCM (NOLOAD) : { __DTCM_start = .; KEEP(*(DTCM)) KEEP(*(DTCM.*)) __DTCM_end = .; } > DTCM __DTCM_size = __DTCM_end - __DTCM_start; __DTCM_load_start = LOADADDR(DTCM); ITCM (NOLOAD) : { __ITCM_start = .; KEEP(*(ITCM)) KEEP(*(ITCM.*)) __ITCM_end = .; } > ITCM __ITCM_size = __ITCM_end - __ITCM_start; __ITCM_load_start = LOADADDR(ITCM);
.last_section :
{
  LONG(0xE015E015)
} > FLASH
